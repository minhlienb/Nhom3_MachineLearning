...
def get_predictions(pred, threshold=0.1, objects=None):
    predicted_classes = [
        (COCO_INSTANCE_CATEGORY_NAMES[i], p, [(box[0], box[1]), (box[2], box[3])], mask)
        for i, p, box, mask in zip(pred[0]['labels'].numpy(), pred[0]['scores'].detach().numpy(),
                                   pred[0]['boxes'].detach().numpy(), pred[0]['masks'].detach().numpy())
    ]
    predicted_classes = [stuff for stuff in predicted_classes if stuff[1] > threshold]
    if objects and predicted_classes:
        predicted_classes = [(name, p, box, mask) for name, p, box, mask in predicted_classes if name in objects]
    return predicted_classes

# Function to draw bounding boxes and masks
def draw_box(pred_class, img, rect_th=2, text_size=0.5, text_th=2, img_name="hinh.png"):
    ...
    for predicted_class in pred_class:
        label, probability, box, mask = predicted_class
        ...
        # Draw mask
        mask = mask[0, :, :] > 0.5  # Thresholding the mask
        color = np.random.randint(0, 255, (3,), dtype=int)  # Random color for mask
        
        # Apply mask color to the image
        for c in range(3):  # Apply color to each channel (R, G, B)
            image[:, :, c][mask] = np.clip(0.5 * image[:, :, c][mask] + 0.5 * color[c], 0, 255)
